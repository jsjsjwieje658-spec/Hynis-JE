...
public class STBImageResize {
    ...
    // --- [ stbir_resize_uint8_linear ] (LWJGL 3.4 API, backported) ---
    // Minecraft 26.1 calls this when entering a world (takeAutoScreenshot resizes
    // the framebuffer for the pause-menu background) and for some image loading.
    // The bundled stb dylib predates the function, so it is reimplemented here in
    // pure Java with plain bilinear sampling. It is only used for UI backgrounds
    // and skin/thumbnail scaling, so filter quality is not critical.

    /**
     * Unsafe version of: {@link #stbir_resize_uint8_linear resize_uint8_linear}.
     *
     * <p>Backport note: performs a bilinear resize from {@code input_pixels} into
     * {@code output_pixels} (which must be non-{@code NULL}, as Minecraft always
     * provides it) and returns {@code output_pixels}.</p>
     */
    public static long nstbir_resize_uint8_linear(long input_pixels, int input_w, int input_h, int input_stride_in_bytes, long output_pixels, int output_w, int output_h, int output_stride_in_bytes, int pixel_type) {
        int channels = pixelLayoutChannels(pixel_type);

        if (input_stride_in_bytes == 0) {
            input_stride_in_bytes = input_w * channels;
        }
        if (output_stride_in_bytes == 0) {
            output_stride_in_bytes = output_w * channels;
        }

        // Scale factors map the CENTER of output pixels to input space.
        double xScale = input_w == 0 ? 0.0 : (double)input_w / output_w;
        double yScale = input_h == 0 ? 0.0 : (double)input_h / output_h;
        ...
        for (int y = 0; y < output_h; y++) {
            ...
            int y1 = y0 + 1;
            if (y0 < 0) { y0 = 0; fy = 0.0; }
            if (y1 >= input_h) { y1 = input_h - 1; }
            if (y0 > y1) { y0 = y1; }

            long row0 = input_pixels + (long)y0 * input_stride_in_bytes;
            long row1 = input_pixels + (long)y1 * input_stride_in_bytes;
            long outRow = output_pixels + (long)y * output_stride_in_bytes;
            ...
            for (int x = 0; x < output_w; x++) {
                ...
                int x1 = x0 + 1;
                if (x0 < 0) { x0 = 0; fx = 0.0; }
                if (x1 >= input_w) { x1 = input_w - 1; }
                if (x0 > x1) { x0 = x1; }

                long p00 = row0 + (long)x0 * channels;
                long p10 = row0 + (long)x1 * channels;
                long p01 = row1 + (long)x0 * channels;
                long p11 = row1 + (long)x1 * channels;
                long dst = outRow + (long)x * channels;

                for (int c = 0; c < channels; c++) {
                    double top = (memGetByte(p00 + c) & 0xFF) * (1.0 - fx) + (memGetByte(p10 + c) & 0xFF) * fx;
                    double bot = (memGetByte(p01 + c) & 0xFF) * (1.0 - fx) + (memGetByte(p11 + c) & 0xFF) * fx;
                    int v = (int)Math.round(top * (1.0 - fy) + bot * fy);
                    if (v < 0) { v = 0; }
                    if (v > 255) { v = 255; }
                    memPutByte(dst + c, (byte)v);
                }
            }
        }
    ...
    private static int pixelLayoutChannels(int pixel_type) {
        switch (pixel_type) {
            case 1: case 2: case 4: case 6: case 7: case 8:
            case 11: case 12: case 13: case 14: case 15: case 16:
                return 4; // RGBA-style layouts used by Minecraft
            case 3: case 5:
                return 3; // RGB-style
            default:
                return 4; // 0 (=BGR in stb's enum, 4 channels) and unknown: assume 4
        }
    }
